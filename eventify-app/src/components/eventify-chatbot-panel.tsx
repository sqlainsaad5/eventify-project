...
import { Send, Brain, Bot, User, Eraser } from "lucide-react"
...
const initialMessages: Message[] = [
  {
    ...
    role: "assistant",
    content:
      "Greetings! I'm your Eventify Intelligence partner. I can help you optimize your budget, find the perfect vendors, or suggest themes for your upcoming events. What's on your mind today?",
    timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
  },
]

type EventifyChatbotPanelProps = {
  className?: string
}

export function EventifyChatbotPanel({ className }: EventifyChatbotPanelProps) {
    ...
    try {
      ...
      const aiMessage: Message = {
        ...
        content:
          data.reply ||
          data.error ||
          "I encountered a slight anomaly while processing your request. Could you rephrase that?",
        timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
      }
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: "assistant",
          content:
            "Connection to the Eventify Core was lost. Please ensure the backend is active.",
          timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
        },
      ])
    } finally {
  ...
  return (
    <div
      className={cn(
        "flex flex-col max-w-5xl mx-auto w-full",
    ...
    >
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="flex h-2 w-2 rounded-full bg-emerald-500 animate-pulse border border-white shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                Active • GPT-4 Advanced
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 min-h-0 flex flex-col bg-white rounded-[40px] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
        <div className="flex-1 overflow-y-auto px-6 py-10 space-y-8 scrollbar-hide">
          {messages.map((msg) => (
            ...
            >
              <div
                className={cn("shrink-0 p-1.5 rounded-xl border", {
              ...
              <div
                className={cn("flex flex-col gap-1.5 max-w-[75%]", {
                  "items-end": msg.role === "user",
                })}
              >
                <div
                  className={cn("px-5 py-3.5 rounded-[24px] shadow-sm", {
        ...
        <div className="p-8 border-t border-slate-50 bg-slate-50/20 backdrop-blur-md shrink-0">
